...
The parameter values are listed in \cref{tab:chart-parameters-subtask-1}.
...
multiple comparisons (\cref{tab:subtask-1:t-tests-2}).
...
models.
\Cref{fig:2:box-plot} indicates that individual \rlmshort{} and \rlmshorttemp{} models
...
As in \cref{sec:subtask-1}, I performed paired $t$-tests and the Nemenyi test on the
...
\begin{table}
  ...
  \caption{The $t$-statistics from paired $t$-tests, and $p$-values from the Nemenyi test, on the
    ...
    As in \cref{tab:subtask-1:t-tests}, a positive $t$-statistic indicates that `Model B'
    achieved a lower mean score than `Model A' and vice versa.
  }
  \label{tab:subtask-2:t-tests}
\end{table}

Finally, I performed the same statistical tests for the best estimators for
sub-task~1~(b) and sub-task~2 (\cref{tab:subtasks:friedman}).
The results were consistent with those in
\cref{tab:subtask-1:t-tests-2,tab:subtask-2:t-tests}, i.e., the following model classes
...
(\cref{tab:subtask-1:results-2,tab:subtask-2:results}).
...
\begin{landscape}
  \begin{table}
    ...
    \caption{The $t$-statistics from paired $t$-tests, and $p$-values from the Nemenyi test, on the
      ...
      score than `Model A' and vice versa.
    }
    \label{tab:subtasks:friedman}
  \end{table}
\end{landscape}
